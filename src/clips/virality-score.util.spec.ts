...
describe('calculateViralityScore', () => {
  ...
  it('score is clamped to 100 maximum', () => {
    const score = calculateViralityScore({
      durationSeconds: 30,
      positionRatio: 0.5,
      transcript: 'amazing incredible shocking unbelievable secret hack trick tip mistake never',
    });
    expect(score).toBeLessThanOrEqual(100);
  });

  it('score is never below 0', () => {
    const score = calculateViralityScore({ durationSeconds: 0, positionRatio: 0 });
